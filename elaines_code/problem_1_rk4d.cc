...
p1_rk4d::p1_rk4d(int dof_) : dof(dof_), fcount(0), t(0.), q(new double[dof]),
...
void p1_rk4d::print(double t_,double *in) {
    printf("%g",t_);
    for(int i=0;i<dof;i++) {
        printf(" %g",in[i]);
    }
...
void p1_rk4d::solve_fixed(double duration,double Atol,bool output,int d_steps) {
        ...
        if (err>1. and last) {// last step but step is rejected
            ...
            if(d_steps>0) {
                while(t_den+dt_den<t) {
                    ...
                    if ((fabs(t_den-10.) < 1e-12) or (fabs(t_den-20.) < 1e-12) or (fabs(t_den-50.) < 1e-12) or (fabs(t_den-200.) < 1e-12)) {
                        print(t_den,k3);
                    }
                }
            }
...
void p1_rk4d::dense_output(double theta,double dt) {
    double mth=1-theta;
